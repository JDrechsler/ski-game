...
const IMAGES_JUMP: IMAGE_NAMES[] = [IMAGE_NAMES.SKIER_JUMP1];
...
export class Skier extends Entity {
  ...
  isInAir(): boolean {
    return (
      ...
      this.state === STATES.STATE_FLIPPING
    );
  }
   ...
   */
  handleInput(inputKey: string, inputState: KEY_STATES) {
    ...
    switch (inputKey) {
      case KEYS.LEFT:
        inputState === KEY_STATES.PRESS ? this.turnLeft() : this.turnDown();
        break;
      case KEYS.RIGHT:
        inputState === KEY_STATES.PRESS ? this.turnRight() : this.turnDown();
        break;
      case KEYS.SPACE:
        this.jump();
      default:
  ...
  checkIfHitObstacle() {
    ...
    if (collision) {
      ...
      if (this.isInAir()) {
        if (
          collision.imageName === IMAGE_NAMES.TREE ||
          collision.imageName === IMAGE_NAMES.TREE_CLUSTER
        ) {
          if (this.isJumping()) {
            this.crash();
          }
